...
// fastxml ObjectMapper에서 Class정보를 이용하여 리플렉션 할 때 기본생성자로 생성함.
// 매개변수가 있는 생성자 : 매개변수의 정보를 알수 없기 때문에 자동생성 불가. 이게있다면 기본생성자을 강제로 만들어야 됨.
@Entity
@Table(name = "board")
@Getter
@NoArgsConstructor(access = AccessLevel.PROTECTED) // new Board() 작성이 불가 -> 객체의 일관성 유지에 좋음
public class Board {
    @Id
